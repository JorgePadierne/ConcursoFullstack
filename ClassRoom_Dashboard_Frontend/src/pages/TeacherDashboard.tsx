...
const TeacherDashboard: React.FC = () => {
  const { user } = useAuth();
  const [courseSummaries, setCourseSummaries] = useState<TeacherCourseSummary[]>([]);
  ...
  const totalTasks = courseSummaries.reduce((sum, course) => sum + course.tasks, 0);
  const totalSubmissions = courseSummaries.reduce((sum, course) => sum + course.submissions, 0);
  const totalCompleted = courseSummaries.reduce((sum, course) => sum + course.handedIn, 0);
        ...
        <div className="divide-y divide-gray-200">
          {courseSummaries.length > 0 ? (
            courseSummaries.map((course) => {
              const completionRate = course.tasks > 0 ? (course.handedIn / course.tasks) * 100 : 0;
                    ...
                    <div className="flex-1">
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-500">{courseInfo?.section}</p>
                        </div>
                      </div>
                      <div className="mt-3 grid grid-cols-3 gap-4">
                        <div>
                          <p className="text-xs text-gray-500">Assignments</p>
                          <p className="text-sm font-medium text-gray-900">{course.tasks}</p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-500">Submissions</p>
                          <p className="text-sm font-medium text-gray-900">{course.submissions}</p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-500">Completed</p>
                          <p className="text-sm font-medium text-gray-900">{course.handedIn}</p>
                        </div>
                      </div>
                    </div>
                    <div className="ml-6">
                      <div className="text-right">
                        <p className="text-sm font-medium text-gray-900">{completionRate.toFixed(1)}%</p>
            ...
            <div className="px-6 py-8 text-center">
              ...
              <p className="mt-1 text-sm text-gray-500">Start by creating courses in Google Classroom.</p>
            </div>
          )}
        </div>
      </div>
